...
bool SerialPacketsDecoder::process_packet() {
  ...
  if (packet_crc != computed_crc) {
    ...
    logger.error("Incoming packet has bad CRC: %04hx vs %04hx",
                 packet_crc, computed_crc);
    return false;
  }
  ...
  if (packet_type == TYPE_COMMAND) {
    if (_packet_len < 8) {
      logger.error("Incoming command packet is too short: %hu",
                   _packet_len);
      return false;
    }
  ...
  if (packet_type ==  TYPE_RESPONSE) {
    if (_packet_len < 8) {
      logger.error("Incoming response packet is too short: %hu",
                   _packet_len);
      return false;
    }
  ...
  if (packet_type ==  TYPE_MESSAGE) {
    if (_packet_len < 4) {
      logger.error("Incoming message packet is too short: %hu",
                   _packet_len);
      return false;
    }
