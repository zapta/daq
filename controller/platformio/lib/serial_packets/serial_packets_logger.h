...
//   void set_stream(Serial* stream) { _optional_stream = stream; }
...
//     return   std::max(SERIAL_PACKETS_LOG_VERBOSE, std::min(SERIAL_PACKETS_LOG_NONE, level));
//     // constrain(level, SERIAL_PACKETS_LOG_VERBOSE, SERIAL_PACKETS_LOG_NONE);
//   }
