...
typedef void (*SerialPacketsIncomingCommandHandler)(
    uint8_t command_endpoint, const SerialPacketsData& command_data,
...
typedef void (*SerialPacketsIncomingMessageHandler)(
    uint8_t message_endpoint, const SerialPacketsData& message_data);
...
class SerialPacketsClient {
 public:
  ...
  bool sendCommand(uint8_t endpoint, const SerialPacketsData& data,
                   uint32_t user_data,
 ...
 private:
  ...
  void force_next_pre_flag() {
    _prot.pre_flag_timer.set(serial_packets_consts::PRE_FLAG_TIMEOUT_MILLIS + 1);
  }
  ...
  CommandContext* find_context_with_cmd_id(uint32_t cmd_id) {
    for (int i = 0; i < MAX_PENDING_COMMANDS; i++) {
      if (_prot.command_contexts[i].cmd_id == cmd_id) {
        return &_prot.command_contexts[i];
      }
    }
    return nullptr;
  }
