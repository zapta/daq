    ...
    def prune_older_than(self, min_time: float):
        ...
        for i in range(len(self.markers)):
            if self.markers[i].time >= min_time:
                ...
                break
        if items_to_delete:
            self_markers = self_markers[items_to_delete:]
...
async def message_async_callback(endpoint: int, data: PacketData) -> Tuple[int, PacketData]:
    ...
    global lc_channels, therm_channels, last_log_message_time_millis
    ...
    if endpoint == 10:
        ...
        if packet_end_time and packet_end_time > latest_log_time:
        ...
        if marker_data:
            for time_millis, marker_name in marker_data.timed_values():
                ...
                markers_hostory.append(time, marker_name)
...
def update_display():
    ...
    plot1.clear()
    plot2.clear()
    markers_hostory.keep_at_most(50)

    if not latest_log_time:
        logger.info("No log base time to update graphs")
        return

    # TODO: use const for 200sec. This is the oldest time plot1 or
    # plot2 can show.
    markers_hostory.prune_older_than(latest_log_time - 200.0)
    ...
    for marker_time, marker_name in markers_hostory.markers:
        rel_time = marker_time - latest_log_time
        plot1.addLine(x=rel_time)
        plot2.addLine(x=rel_time)
